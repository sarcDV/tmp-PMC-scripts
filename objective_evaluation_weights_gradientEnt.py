...
"""
# pd_off_ = pd.read_csv('./AES_analysis/pd_OFF.txt', header=None)
# test_ = (pd_off_[0][20]).split(" ")
# print(test_[1])
...
ax1 = plt.subplot(111)
bp4 = ax1.boxplot([ np.float64(t1_off_),  np.float64(t2_off_), np.float64(pd_off_),
                   np.float64(t2s05_off_), np.float64(t2s035_off_), np.float64(t2s025_off_)], 
...
bp5 = ax1.boxplot([ np.float64(t1_on_),  np.float64(t2_on_), np.float64(pd_on_),
                   np.float64(t2s05_on_), np.float64(t2s035_on_), np.float64(t2s025_on_)],  
...
d_t1_ = {
     'Score':     np.float64(np.concatenate((t1_off_, t1_on_))),
...
d_t2pd_ = {
     'Score':     np.float64(np.concatenate((t2_off_, t2_on_, pd_off_, pd_on_))),
...
d_t2s_ = {
     'Score':     np.float64(np.concatenate((t2s05_off_, t2s05_on_,
